...
export default function QRCodeManagementPage() {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const response = await api.get('/siswa', { params: { per_page: 1000 } }); // Get all students
      
      // Handle paginated response: response.data.data.data
      const responseData = response.data;
      const studentData = responseData.data?.data || responseData.data || [];
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={async () => {
                    try {
                      await downloadQRCode(selectedStudent.nis);
                    } catch (error) {
