...
export default function TambahTugasAdabPage() {
  ...
  const fetchTahunAjaran = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const aktiveTahun = response.data.data?.find((tahun: TahunAjaran) => tahun.status === 'Aktif');
        if (aktiveTahun) {
          setFormData(prev => ({
            ...prev,
            id_tahun_ajaran: aktiveTahun.id_tahun_ajaran.toString()
          }));
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/v1/tugas-adab', {
        ...formData,
      ...
      if (response.data.success) {
        ...
        if (response.data.errors) {
          ...
          alert(response.data.message || 'Gagal menyimpan tugas adab');
        }
      }
    } catch (error) {
      console.error('Error saving tugas adab:', error);
      alert('Terjadi kesalahan saat menyimpan tugas adab');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Buat tugas adab baru untuk siswa</p>
          </div>
        </div>
      </div>
