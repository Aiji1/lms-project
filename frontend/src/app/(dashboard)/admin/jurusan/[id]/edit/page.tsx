...
interface EditJurusanPageProps {
  params: Promise<{
    id: string;
  }>;
}
...
export default function EditJurusanPage({ params }: EditJurusanPageProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data.success) {
        const data = response.data.data.jurusan;
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.put(`/v1/jurusan/${id}`, formData);
      
      if (response.data.success) {
        alert('Jurusan berhasil diperbarui!');
        router.push(`/admin/jurusan/${id}`);
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Jurusan</h1>
          <p className="text-gray-600">Perbarui data master jurusan</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Masukkan nama jurusan (contoh: Tahfizh, Digital, IPA, dll)
              </p>
            </div>
