...
export default function EditTahunAjaranPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data.success) {
        const data = response.data.data.tahun_ajaran;
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.put(`/v1/tahun-ajaran/${id}`, formData);
      
      if (response.data.success) {
        alert('Tahun ajaran berhasil diperbarui!');
        router.push(`/admin/tahun-ajaran/${id}`);
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Tahun Ajaran</h1>
          <p className="text-gray-600">Perbarui data tahun ajaran</p>
        </div>
      </div>
