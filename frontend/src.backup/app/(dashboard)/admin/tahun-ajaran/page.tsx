...
export default function TahunAjaranPage() {
  ...
  const handleDelete = async (item: TahunAjaran) => {
    if (!confirm('Apakah Anda yakin ingin menghapus tahun ajaran ini?')) {
      return;
    }

    try {
      const response = await api.delete(`/v1/tahun-ajaran/${item.id_tahun_ajaran}`);
      if (response.data.success) {
        fetchData();
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.download = `template_tahun_ajaran_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <div className="bg-white rounded-lg shadow-sm border border-black p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Kelola data tahun ajaran dan semester</p>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item.id_tahun_ajaran} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{item.tahun_ajaran}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{item.semester}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        <Link
                          href={`/admin/tahun-ajaran/${item.id_tahun_ajaran}`}
                        ...
                        <Link
                          href={`/admin/tahun-ajaran/${item.id_tahun_ajaran}/edit`}
