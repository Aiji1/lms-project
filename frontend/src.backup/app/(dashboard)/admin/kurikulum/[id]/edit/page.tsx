...
interface EditKurikulumPageProps {
  params: Promise<{
    id: string;
  }>;
}
...
export default function EditKurikulumPage({ params }: EditKurikulumPageProps) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setFormData({
          ...
          rombel: data.rombel || '',
          status: data.status || '',
          sks_jam_perminggu: data.sks_jam_perminggu?.toString() || ''
        });
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.put(`/v1/kurikulum/${id}`, submitData);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Kurikulum</h1>
          <p className="text-gray-600">Perbarui data kurikulum</p>
        </div>
      </div>
